...
// 진하게 칠해진 영역이 채팅이다.
function LayoutGlyph(props: { mode: WorkspaceLayoutModeId }) {
  const { mode } = props;
  const showTree =
...
function WorkspaceLayoutMenu(props: {
  mode: WorkspaceLayoutModeId;
  onSelect: (mode: WorkspaceLayoutModeId) => void;
  buttonClassName: string;
}) {
  const active =
    WORKSPACE_LAYOUT_MODES.find((mode) => mode.id === props.mode) ??
    WORKSPACE_LAYOUT_MODES[0];
  return (
...
export function ProjectWorkspace(props: ProjectWorkspaceProps) {
  ...
  const [layoutMode, setLayoutMode] =
    useState<WorkspaceLayoutModeId>('default');
  ...
  const centerHidden = isWorkspaceCenterHidden(
    layoutMode,
