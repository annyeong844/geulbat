...
>[0]['runSession'];
type HomeFilesInput = ReturnType<typeof createHomeFilesInput>;
type HomeThreadsInput = ReturnType<typeof createHomeThreadsInput>;
...
function createPropsStub(): HomeShellProps {
  return {
...
function createFilesStub(): HomeFilesInput {
  return {
...
function createThreadsStub(): HomeThreadsInput {
...
void test('createHomeShellView composes panel views from files, threads, provider auth, and run session inputs', () => {
  ...
  const shell = createHomeShellView({
    ...createPropsStub(),
...
void test('createHomeShellView edits a past question by branching before it and rerunning', async () => {
...
void test('createHomeShellView skips the rerun when the edit branch fails', async () => {
