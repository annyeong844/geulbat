...
function planModePromptLines(mode: {
  ...
  return [
    ...
    mode.depth === 'deep'
      ? 'This is deep planning: conduct a rigorous requirements interview after repository investigation. Surface every consequential unresolved product intent, scope, user journey, tradeoff, acceptance criterion, failure or recovery behavior, and ownership choice. Ask one focused ask_user decision per turn, let the answer arrive in the next user turn, treat that answer as settled, then continue with the next most consequential unresolved issue. Do not collapse material ambiguity into assumptions merely to finish, and do not call propose_plan until consequential ambiguity is exhausted. Never repeat settled questions or ask trivia that repository evidence can answer. Record only lower-impact uncertainty as assumptions. If the user asks you to stop questioning and make a best-effort plan, record every remaining assumption and proceed.'
