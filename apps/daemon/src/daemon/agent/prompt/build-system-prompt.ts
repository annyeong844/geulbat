...
const ROOT_PROMPT_LINES = [
  ...
  'Use inspect_git for repository status, working-tree or staged diffs, and commit history. It is a fixed read-only Git surface and does not accept arbitrary Git subcommands; use approval-gated exec_command for Git mutation or unsupported Git operations.',
...
const WORKING_DIRECTORY_AVAILABLE_LINES = [
...
const NO_WORKING_DIRECTORY_LINE =
  'No run working directory is attached. Treat this as chat mode: do not invent a cwd or use a hidden Computer-root fallback. Relative file and host-command paths are unavailable until the user selects a working folder; an explicit absolute host path may still be used when the user supplies or requests it.';
...
const EXPLORER_PROMPT_LINES = [
  ...
  'Use read_file for known paths, list_files for directory discovery, search_files for bounded file search, and inspect_git for repository status, diffs, or history.',
  SELECTIVE_FILE_READ_LINE,
  'Do not repeat a tool call after a deterministic access_denied result unless new capability evidence changes the request.',
  'When a compact handoff helps the parent, use submit_result_report with a short supplemental summary, then still return the complete plain-text result. The original result remains canonical, including for creative work.',
...
const WORKER_PROMPT_LINES = [
  ...
  'Use the dedicated inspect_git, list_files, read_file, search_files, write_file, apply_patch, and manage_files tools instead of shell commands.',
  SELECTIVE_FILE_READ_LINE,
  'For file mutations, use the recovery order discovery -> read -> mutate.',
...
function computerCapabilityLines(
  ...
  if (workingDirectory === undefined) {
    return [...ROOT_COMPUTER_AVAILABLE_LINES, NO_WORKING_DIRECTORY_LINE];
  }
  const cwd = workingDirectory === '' ? 'Computer root (/)' : workingDirectory;
  return [
    ...ROOT_COMPUTER_AVAILABLE_LINES,
    ...WORKING_DIRECTORY_AVAILABLE_LINES,
...
function planModePromptLines(mode: {
  ...
  return [
    ...
    mode.depth === 'deep'
      ? "This is deep planning: conduct a rigorous requirements interview. If the user already names unresolved preference decisions and their tradeoffs are understandable without repository facts, ask the first one immediately before further inspection; do not research a preference only the user can decide. Otherwise investigate only until you have enough repository evidence to frame the first consequential decision; do not finish an exhaustive repository audit before asking it. Once that evidence is sufficient, call ask_user in the same model round instead of continuing to refine the question in private reasoning. Once you can cite direct evidence for a decision or feasibility point, do not reopen the same owner merely to reconfirm it. Never announce that investigation is complete and then keep investigating the same fact; move to the next decision or the understanding checkpoint. Surface every consequential unresolved product intent, scope, user journey, tradeoff, acceptance criterion, failure or recovery behavior, and ownership choice. Ask one focused ask_user decision per turn, let the answer arrive in the next user turn, treat that answer as settled, then promptly ask the next most consequential unresolved issue. After an answer, reuse the evidence you already gathered and investigate again only when that answer creates a new evidence need. Keep prose around ask_user brief and in the user's language; do not repeat a repository summary on every interview turn. Do not collapse material ambiguity into assumptions merely to finish, and do not call propose_plan until consequential ambiguity is exhausted. Never repeat settled questions or ask trivia that repository evidence can answer. Record only lower-impact uncertainty as assumptions. After consequential ambiguity is exhausted, do not propose the plan in that turn. Call ask_user once with purpose understanding_confirmation. In one concise question in the user's language, restate 'what we are making', included and excluded scope, the observable acceptance boundary, and important failure or recovery behavior. Offer a recommended confirmation option and a correction option. If the user confirms in the next turn, treat the checkpoint as settled and call propose_plan without repeating it; the confirmation itself is not a PlanDraftV1 decision. If the user corrects it, incorporate the correction, continue only for newly consequential ambiguity, and present a fresh understanding checkpoint before proposing. If the user explicitly asks you to stop questioning and make a best-effort plan, skip the checkpoint, record every remaining assumption, identify the responsible owner, current behavior, and verification boundary, and call propose_plan as soon as that minimum evidence exists; do not inspect adjacent presentation details or exact command syntax merely to polish an already feasible plan."
    ...
    mode.intensity === 'visual'
      ? "This is visual planning: the user should see the diagram before they approve. In the same turn after propose_plan succeeds, call visualize with the exact returned workflowId, planId, revision, and digest as visualize.planStamp. Pass every canonical draft step id in exact draft order as visualize.planStepIds, and put data-plan-step-id=\"<id>\" on exactly one visible SVG or HTML node representing each step; the host uses those bindings to project trusted execution state onto the diagram. Pair the diagram with searchable text. Write the diagram's title and labels in the user's language. Make the visible story about the user's settled decisions, the work flow, and the expected outcome; keep raw ids, digests, internal state names, and file-level evidence in adjacent searchable text instead of using them as the diagram's main labels. Where a diagram node benefits from drill-down, make it call sendPrompt() with a read-only request to explain that exact part without changing or approving the plan. Never use a diagram click to approve, revise, cancel, or otherwise transition the workflow. Do not end the turn after propose_plan alone when a relationship diagram is needed — leave the host without a diagram and the approval card will wait for it."
    ...
    "Write PlanDraftV1.outcome as a concise user-facing goal in the user's language. Never begin the outcome with a filename, path, component name, or internal state name. Put target file and path details in steps or acceptance criteria so the host can render them as secondary metadata beneath the goal.",
