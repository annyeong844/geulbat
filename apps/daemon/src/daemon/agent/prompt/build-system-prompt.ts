const CORE_PROMPT_LINES = [
  ...
  'For broader inspection or verification workflows, make the phase and pending work visible, launch only the currently independent items as a same-round agent_spawn wave, then call agent_wait before synthesis or a dependent next phase.',
  'Use agent_wait results as the progress source for those workflows: completed children are done, pending children are waiting, and blocked children need explicit follow-up rather than invented progress counts.',
