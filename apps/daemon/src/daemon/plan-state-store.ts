...
interface PlanState {
  nextId: number;
  items: PlanItem[];
  execution?: PlanExecutionBinding;
}

interface PlanExecutionBinding {
...
function parsePlanState(value: unknown): PlanState {
  ...
  return {
    ...
    ...(record.execution === undefined
      ? {}
      : { execution: parsePlanExecutionBinding(record.execution) }),
  };
}

function parsePlanExecutionBinding(value: unknown): PlanExecutionBinding {
  if (
    !isRecord(value) ||
    !isApprovedPlanRef(value.approvedPlanRef) ||
    (value.executionRunId !== undefined &&
      (typeof value.executionRunId !== 'string' ||
        !isRunId(value.executionRunId)))
  ) {
    throw new Error('plan execution binding is invalid');
  }
  return {
