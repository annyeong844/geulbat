...
void test('parsePageLoadEvidenceStdout preserves title text longer than the legacy maxChars field', () => {
  const longTitle = 'Long page title '.repeat(20);
  const result = parsePageLoadEvidenceStdout({
    stdout: JSON.stringify({
      ok: true,
      capability: PTC_LAB_BROWSER_PAGE_LOAD_EVIDENCE_CAPABILITY,
  ...
  assert.equal(result.ok ? result.value.ok : false, true);
  assert.equal(
    result.ok && result.value.ok && result.value.title
      ? result.value.title
      : '',
    longTitle,
  );
