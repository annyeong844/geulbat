...
type PtcLabBrowserUserUrlAdmissionReasonCode =
  | 'accepted'
  | 'url_not_string'
  | 'url_empty'
  | 'url_too_large'
...
type PtcLabBrowserUserUrlAdmissionFailureReasonCode = Exclude<
  PtcLabBrowserUserUrlAdmissionReasonCode,
  'accepted'
>;

type PtcLabBrowserUserUrlUnsupportedFieldCategory =
  | 'browser_execution'
  | 'browser_evidence'
  | 'browser_action'
  | 'browser_crawler'
...
export type PtcLabBrowserUserUrlTargetDigest = PtcSha256Digest;
...
export interface PtcLabBrowserValidatedUrlRequest {
  target: PtcLabBrowserUserUrlNavigationTarget;
  timeoutMs: number;
}

type PtcLabBrowserUserUrlAdmissionResult<T> =
  | { ok: true; value: T }
  | {
      ok: false;
...
function digestPtcLabBrowserUserUrlNavigationTarget(
  target: PtcLabBrowserUserUrlNavigationTargetDigestInput,
): PtcLabBrowserUserUrlTargetDigest {
  return digestPtcStableJson(target, { omitUndefinedObjectKeys: true });
}
