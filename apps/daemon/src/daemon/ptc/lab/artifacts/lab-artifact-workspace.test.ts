...
void test('importPtcLabArtifactWorkspaceFile imports one explicit file into sandbox evidence', async () => {
  await withTempRoots(async ({ workspaceRoot, artifactRoot }) => {
    ...
    assert.equal(JSON.stringify(attempts).includes(artifactRoot), false);
    assert.match(
...
void test('importPtcLabArtifactWorkspaceFile rejects a symlink artifact root without reading its target', async () => {
  await withTempRoots(async ({ workspaceRoot, artifactRoot }) => {
    ...
    try {
      await mkdir(join(outsideRoot, 'out'), { recursive: true });
      await writeFile(join(outsideRoot, 'out', 'result.txt'), 'outside');
      await rm(artifactRoot, { recursive: true, force: true });
      await symlink(outsideRoot, artifactRoot);

      for (const rootPath of [artifactRoot, `${artifactRoot}/`]) {
        const store = createSandboxAttemptStore();
        const result = await importPtcLabArtifactWorkspaceFile({
          admission: admittedLab(),
        ...
        assert.equal(store.getAttempts().records[0]?.status, 'failed');
        assert.equal(store.getAttempts().records[0]?.outputRef, null);
      }
    } finally {
      await rm(outsideRoot, { recursive: true, force: true });
    }
