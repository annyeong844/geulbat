...
export function validatePackageInstallRequest(args: {
  ...
  const timeout = admitPtcBoundedTimeoutMs({
    ...
    defaultTimeoutMs: args.policy.packageManager.maxInstallMs,
    maxTimeoutMs: args.policy.packageManager.maxInstallMs,
  });
  if (!timeout.ok) {
    return requestInvalid();
  }
...
function validateCommonPackageInstallSession(args: {
  ...
  if (
    ...
    !isPtcSha256Hex(args.session.packageCacheIdentityHash)
  ) {
    return packageInstallFailure(
      'ptc_lab_package_install_policy_mismatch',
