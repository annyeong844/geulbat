...
} from './lab-package-cache-root.js';
import {
...
void test('cleanupPtcPackageCacheRoot fails closed when cache namespace cannot be resolved', async () => {
  await withTempRuntimeRoot(async (runtimeRoot) => {
    const identity = normalizePtcPackageCacheIdentity(BASE_IDENTITY);
    await preparePtcPackageCacheRoot({ runtimeRoot, identity });
    await rm(join(runtimeRoot, 'ptc-package-caches'), {
      recursive: true,
      force: true,
    });
    ...
    assert.deepEqual(cleanup.ok ? {} : cleanup.diagnostics, {
      cleanupFailed: true,
    });
...
void test('cleanupPtcPackageCacheRoot rejects non-canonical cache identity token forms', async () => {
  await withTempRuntimeRoot(async (runtimeRoot) => {
    const invalidIdentities = [
      ...
      normalizePtcPackageCacheIdentity({
        ...BASE_IDENTITY,
        labPolicyId: 'ptc.lab.local.docker.policy.v1',
      }),
      normalizePtcPackageCacheIdentity({
        ...BASE_IDENTITY,
        packageCacheId: 'ptc:lab:package:cache:local:v1',
      }),
    ];
