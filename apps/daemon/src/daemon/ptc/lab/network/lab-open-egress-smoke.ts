...
import { buildPtcLabPublicSessionId } from '../shell/lab-session-public-id.js';
...
const PTC_LAB_OPEN_EGRESS_SMOKE_POLICY_ID =
  'ptc_lab_open_egress_smoke_v1' as const;
const PTC_LAB_OPEN_EGRESS_SMOKE_TARGET_POLICY_ID =
...
type PtcLabOpenEgressSmokeFailureReason =
...
type PtcLabOpenEgressSmokeResult<T> =
  | { ok: true; value: T }
  | {
      ok: false;
...
interface PtcLabOpenEgressSmokeRequest {
  smokeId: string;
  timeoutMs?: number;
}

interface PtcLabOpenEgressSmokeSummary {
  ok: true;
  profile: 'lab';
  policyId: string;
  labSessionId: string;
...
interface RunPtcLabOpenEgressSmokeArgs {
...
export async function runPtcLabOpenEgressSmoke(
  ...
  // Inactive diagnostic scaffold: keep as lab-only smoke validation until a
  // product owner promotes it, or delete it if the probe lane is abandoned.
...
function readOpenEgressPolicy(
  ...
  if (labPolicy.value.network.metricsCoverage === 'runtime_observed') {
    return failure(
      'ptc_lab_open_egress_policy_disabled',
...
function validateSmokeRequest(
  ...
  const timeout = admitPtcBoundedTimeoutMs({
    ...
    defaultTimeoutMs: PTC_LAB_OPEN_EGRESS_SMOKE_DEFAULT_TIMEOUT_MS,
    maxTimeoutMs: PTC_LAB_OPEN_EGRESS_SMOKE_MAX_TIMEOUT_MS,
  });
  if (!timeout.ok) {
    return requestInvalid();
  }
  return {
    ok: true,
    value: { smokeId: request.smokeId, timeoutMs: timeout.value },
  };
}
...
async function mapSmokeExecution(args: {
  ...
  if (args.execution.kind === 'timeout') {
    const diagnostics = shouldCloseTaintedPtcDockerSessionForCommandResult(
      args.execution,
    )
      ? toPtcSessionTaintCloseDiagnostics(
  ...
  if (
    args.execution.kind === 'crash' ||
    args.execution.kind === 'output_limit_exceeded'
  ) {
    const taintDiagnostics = shouldCloseTaintedPtcDockerSessionForCommandResult(
      args.execution,
    )
      ? toPtcSessionTaintCloseDiagnostics(
          await closeTaintedPtcDockerSession(args.args),
    ...
    return failure(
      ...
      { commandResultKind: args.execution.kind, ...taintDiagnostics },
    );
  }
...
function parseSmokeStdout(stdout: string): PtcLabOpenEgressSmokeResult<void> {
  ...
  if (!isPtcRecord(parsed) || typeof parsed.ok !== 'boolean') {
    return failure(
      'ptc_lab_open_egress_output_invalid',
...
const failure = ptcFailure<PtcLabOpenEgressSmokeFailureReason>;
