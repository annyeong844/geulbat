...
void test('createPtcLabOpenEgressLocalPolicy rejects unsafe explicit opt-in ids', () => {
  ...
  assert.throws(
    () =>
      createPtcLabOpenEgressLocalPolicy({
        explicitOptInPolicyId: 'PTC_LAB_OPEN_EGRESS_EXPLICIT_LOCAL_V1',
      }),
    /explicit opt-in policy id/u,
  );
  assert.throws(
    () =>
      createPtcLabOpenEgressLocalPolicy({
        explicitOptInPolicyId: 'ptc.lab.open.egress.explicit.local.v1',
      }),
    /explicit opt-in policy id/u,
  );
});
...
void test('doesPtcLabOpenNetworkSessionMatchPolicy rejects lab and network identity drift while keeping metrics coverage telemetry-only', () => {
  ...
  const differentMetricsCoverageNetwork = createPtcLabOpenEgressLocalPolicy({
    metricsCoverage: 'owner_outcome_only',
  });
  if (differentMetricsCoverageNetwork.mode !== 'open') {
    throw new Error('expected open network policy');
  }
  const handle = {
    reuseKey: {
  ...
  const identitySnapshot = handle.reuseKey.network as Record<string, unknown>;
  ...
  assert.equal(Object.hasOwn(identitySnapshot, 'metricsCoverage'), false);
  assert.deepEqual(
    toPtcLabNetworkIdentitySnapshot(differentMetricsCoverageNetwork),
    handle.reuseKey.network,
  );
  assert.equal(
    doesPtcLabOpenNetworkSessionMatchPolicy({
      handle,
      policyId,
      network: differentMetricsCoverageNetwork,
    }),
    true,
  );
  assert.equal(
