...
import { buildPtcLabPublicSessionIdFromIdentityHash } from './lab-session-public-id.js';
...
void test('PTC lab public session id exposes only the stable 32-char identity hash prefix', () => {
  ...
  const id = buildPtcLabPublicSessionIdFromIdentityHash(
    `${visiblePrefix}${hiddenSuffix}`,
  );
