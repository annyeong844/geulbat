...
const ATTEMPT_DIGEST = `sha256:${'a'.repeat(64)}` as `sha256:${string}`;

void test('browser runtime command owner imports only private command and close boundaries', async () => {
  const source = await readFile(
    new URL(
      '../../../src/daemon/ptc/lab-browser-navigation-runtime-command.ts',
      import.meta.url,
    ),
    'utf8',
  );
  ...
  assert.match(source, /session-taint-close\.js/u);
  assert.doesNotMatch(
    source,
    /lab-browser-user-url-navigation(?:-[a-z]+)?\.js/u,
  );
  assert.doesNotMatch(
    source,
    /lab-browser-page-load-evidence(?:-[a-z]+)?\.js/u,
  );
  assert.doesNotMatch(source, /browserResultFailure|PtcLabBrowserResultBase/u);
});
