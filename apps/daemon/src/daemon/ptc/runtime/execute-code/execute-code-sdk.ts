...
function buildStoreSource(args: { callbackAvailable: boolean }): string {
  ...
  return [
    ...
    '    const response = __geulbatValidateCallbackResponse(await __geulbatReadCallbackResponse(socket), requestId);',
...
function buildCallbackSource(
  ...
  return [
    ...
    '  });',
    '}',
    'function __geulbatValidateCallbackResponse(response, requestId) {',
    "  if (response === null || typeof response !== 'object' || Array.isArray(response) || typeof response.ok !== 'boolean') throw new Error('PTC callback response is invalid');",
    "  if (response.requestId !== undefined && response.requestId !== requestId) throw new Error('PTC callback response is invalid');",
    '  if (response.ok === true) {',
    "    if (response.requestId !== requestId) throw new Error('PTC callback response is invalid');",
    '    return response;',
    '  }',
    "  if (typeof response.errorCode !== 'string' || typeof response.message !== 'string' || (response.remediation !== undefined && typeof response.remediation !== 'string') || (response.details !== undefined && (response.details === null || typeof response.details !== 'object' || Array.isArray(response.details)))) throw new Error('PTC callback response is invalid');",
    '  return response;',
    '}',
