...
const ENTRY_URLS_ALLOWED_BY_WEB_SHELL_POLICY = [
  'https://evil.example.com/pkg.js',
  'https://cdn.example.com/app.js',
  'https://192.168.0.1/public-web/react-bundle-counter/entry.js',
] as const;

const INVALID_OR_PRIVILEGED_ENTRY_URLS = [
  './pkg.js',
  'file:///tmp/app.js',
...
void test('acceptReactBundleRuntimeManifest accepts no-dependency manifest without probe evidence', async () => {
  const request: ReactBundleDependencyPrepareRequest = {
    entryUrl:
      'https://cdn.jsdelivr.net/npm/geulbat-runtime-dependency-fixture@1.0.0/no-deps.js',
    runtimeDependencies: {},
    dependencyRefs: [],
  };
...
void test('acceptReactBundleRuntimeManifest preserves tampered entryUrls allowed by web-shell policy', async () => {
  const prepare = await prepareOnly({
  ...
  for (const entryUrl of ENTRY_URLS_ALLOWED_BY_WEB_SHELL_POLICY) {
    ...
    assertOk(result);
    assert.equal(result.manifest.entryUrl, new URL(entryUrl).toString());
  }
});

void test('acceptReactBundleRuntimeManifest rejects tampered invalid or privileged entryUrl before accepted artifact', async () => {
  const prepare = await prepareOnly({
    entryUrl: 'https://fixtures.geulbat.local/no-deps.js',
    runtimeDependencies: {},
    dependencyRefs: [],
  ...
  for (const entryUrl of INVALID_OR_PRIVILEGED_ENTRY_URLS) {
    const result = acceptReactBundleRuntimeManifest({
      prepare: {
        ...prepare,
        manifest: {
...
void test('acceptReactBundleRuntimeManifest rejects duplicate stylesheet manifest dependencies', async () => {
  const request: ReactBundleDependencyPrepareRequest = {
    entryUrl:
      'https://cdn.jsdelivr.net/npm/geulbat-runtime-dependency-fixture@1.0.0/duplicate-stylesheets.js',
    runtimeDependencies: {
      stylesheets: [
        PUBLIC_WEB_REACT_BUNDLE_RUNTIME_DEPENDENCIES_CDN_STYLESHEET_URL,
        PUBLIC_WEB_REACT_BUNDLE_RUNTIME_DEPENDENCIES_CDN_STYLESHEET_URL,
