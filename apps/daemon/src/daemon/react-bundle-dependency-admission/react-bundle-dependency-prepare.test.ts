...
const INVALID_ENTRY_URLS = ['./app.js', ''] as const;

const SHELL_OWNED_PRIVILEGED_ENTRY_URLS = [
...
void test('validateReactBundleDependencyPrepareRequest accepts entryUrl families allowed by web-shell policy', () => {
  for (const entryUrl of ENTRY_URLS_ALLOWED_BY_WEB_SHELL_POLICY) {
    const validated = validateReactBundleDependencyPrepareRequest({
      ...BASE_REQUEST,
      entryUrl,
    });

    assert.equal(validated.entryUrl, new URL(entryUrl).toString(), entryUrl);
  }
});

void test('validateReactBundleDependencyPrepareRequest rejects malformed entryUrl before sandbox execution', () => {
  for (const entryUrl of INVALID_ENTRY_URLS) {
    assert.throws(
      () =>
        validateReactBundleDependencyPrepareRequest({
          ...BASE_REQUEST,
...
void test('validateReactBundleDependencyPrepareRequest rejects non-http entryUrl before sandbox execution', () => {
  for (const entryUrl of UNSUPPORTED_SCHEME_ENTRY_URLS) {
    assert.throws(
      () =>
        validateReactBundleDependencyPrepareRequest({
          ...BASE_REQUEST,
      ...
      /entryUrl must use http or https/,
      entryUrl,
    );
  }
});

void test('validateReactBundleDependencyPrepareRequest rejects shell-owned privileged entryUrl before sandbox execution', () => {
  for (const entryUrl of SHELL_OWNED_PRIVILEGED_ENTRY_URLS) {
    assert.throws(
      () =>
        validateReactBundleDependencyPrepareRequest({
          ...BASE_REQUEST,
...
void test('prepareReactBundleExplicitCdnDependencies preserves extra output files without a hidden collection budget', async () => {
  const workspaceRoot = await mkdtemp(
    join(tmpdir(), 'geulbat-react-bundle-deps-'),
  );
  try {
